...
class BinTree {
   private:
	...
	static void checkFormat(int format) {
		if (format == POSTORDERFORMAT or format == LEFTVISUALFORMAT or format == VISUALFORMAT or
			format == INLINEFORMAT) {
			return;
		}
	...
	static void			  readStringTree(int&													 itoken,
   ...
   public:
	...
	// writing operator <<
	/* Pre: true */
	/* Post: a representation of t is written on os deppending on current value of attribute
	 * 'outputformat' */
	template <class U>
	friend std::ostream& operator<<(std::ostream& os, const BinTree<U>& t);

	// reading operator >>
	// Pre: 'is' contains a representation of a binary tree deppending on current value of attribute
	// 'inputformat'. Post: t is assigned the tree value read from 'is'
	template <class U>
...
template <typename T>
void BinTree<T>::readStringTree(int&													itoken,
								const std::vector<std::pair<std::string, std::string>>& vtoken,
								BinTree<T>&												t) {
...
vector<string> BinTree<T>::generateVisualFormatRec(const BinTree<T>& t) {
	...
	for (int i = 0; i < int(leftresult.size()) and i < int(rightresult.size()); ++i) {
		addspace =
			max(addspace, int(leftresult[i].size()) - firstNonBlankPosInString(rightresult[i]) +
	...
	result.push_back(string(rootpos - rootrelativepos, ' ') + rootstring);
	result.push_back(string(rootpos, ' ') + string(1, '|'));
	result.push_back(string(leftrootpos + 1, ' ') + string(rootpos - leftrootpos - 1, '-') +
					 string(1, ' ') + string(max(0, rightrootpos - rootpos - 1), '-'));
	result.push_back(string(leftrootpos, ' ') + string(1, '|') +
					 string(rightrootpos - leftrootpos - 1, ' ') + string(1, '|'));
